...
public static class OAuthLoopbackHelper
{
    ...
    /// <summary>
    /// Starts an <see cref="HttpListener"/> on <paramref name="redirectUri"/> and waits for
    /// the OAuth provider to redirect back with all token parameters embedded as query-string
    /// fields (i.e. the OAuth 2.0 implicit / token flow, <c>response_type=token</c>).
    /// </summary>
    /// <returns>
    /// A dictionary containing every query-string key/value from the redirect (e.g.
    /// <c>access_token</c>, <c>token_type</c>, <c>hostname</c>, …).
    /// </returns>
    /// <exception cref="InvalidOperationException">
    /// Thrown when the provider returns an <c>error</c> parameter or no
    /// <c>access_token</c>.
    /// </exception>
    public static async Task<Dictionary<string, string>> WaitForTokenRedirectAsync(
        string redirectUri, CancellationToken ct)
    {
        ...
        var token = qs["access_token"]
            ?? throw new InvalidOperationException("Authorization redirect did not include an access_token.");

        // Collect all query-string fields into a plain dictionary.
        var result = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
        foreach (string key in qs.Keys)
        {
            if (key is not null && qs[key] is { } val)
                result[key] = val;
        }
